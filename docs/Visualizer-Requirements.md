...
**Version:** 0.12 (draft) · **Date:** 2026-09-01 · **Status:** For review
...
> **Changes since 0.11:** added a **Flare** effect — a screen-space lens flare driven by whatever the active preset draws brightly, independent of Glow.
> **Changes since 0.10:** the **Text** overlay gained its own **colour** and its own **glow**, and was lifted out of the preset scene so the global Glow no longer reaches it.
...
| Effects | Global glow, lens flare, movement and colour intensity, layered over any preset |
| Text | Optional caption centred over the scene, with size, font, colour and its own glow |
...
Four controls in a panel **Effects** group, applied to whichever preset is active and persisting across preset switches. Each is inert at its default, so a fresh load looks exactly as it did before the group existed.

- **FR-7c Glow** (0–100%, default 0) drives a bloom post-pass. The slider moves strength, luminance threshold and radius together — low settings halo only the hottest cores, high settings pull progressively more of the image into the glow — so the travel across the slider is visible rather than on/off. It glows what the preset draws and must not lift the background. Onsets swell it slightly. It applies to the **preset only**: the text overlay is composited after it and carries its own glow instead (FR-7n).
- **FR-7f Flare** (0–100%, default 0) adds a lens flare derived from the frame itself: ghosts mirrored through the centre of frame and an anamorphic streak, chromatically split and tinted by radius. Nothing is authored per preset — what a preset draws brightly flares, and a preset with nothing hot in it produces nothing. It is generated from the scene **before** the bloom, so it fires whether or not Glow is up and the two remain independent artifacts of the same light rather than one feeding the other. Like Glow, the slider sweeps threshold as well as strength, and onsets swell it slightly, no harder than Glow (NFR-8). It must not lift the background.
...
- **FR-7e Color** (0–200%, default 100%) scales saturation and brightness together: 0 is desaturated and dark, 100% is the preset as authored, 200% is vivid and hot. Unlike Glow, it grades the whole frame, text overlay included.

### 3.2b Text overlay

- **FR-7g** A panel textarea sets an overlay caption. It renders **dead centre**, over everything the preset draws. Empty input renders nothing.
...
- **FR-7k** The caption keeps its position and orientation through zoom, drag-to-look and Movement, and is graded by the Color effect along with the rest of the frame. It is **not** bloomed by the global Glow (FR-7c) — that control belongs to the preset, and the caption has its own (FR-7n).
- **FR-7m Color** — a colour picker sets the caption's fill (default `#ffffff`), with the chosen value shown as hex beside it. It colours the glyphs and their glow together.
- **FR-7n Glow** (0–100%, default 0) haloes the caption alone, independent of the global Glow and inert at 0. Radius and intensity both ramp with the slider, from a tight rim at low settings to a wide corona at high ones. The halo takes the caption's own colour, falls off across **several scales at once** rather than as one blur, sits additively beneath the glyphs, and swells on onsets (damped under reduce-motion, NFR-8).
...
- Shader hygiene (NaN under bloom): a single non-finite fragment is invisible on the direct path, but the bloom's mip chain spreads it across a whole block of the frame, which reads as a black square flickering over the picture. The two ways the shader presets could produce one are now guarded: `fwidth()` collapses to zero on an edge-on or sub-pixel triangle, which makes the barycentric wireframe's `smoothstep(0.0, w, …)` divide by zero, so `w` carries a floor; and `dot()` of two normalised vectors can round to just over 1, handing `pow()` a negative base — undefined — on exactly the silhouette fragments where the fresnel and rim terms are brightest, so every such base is clamped at 0.
...
- Effects: an `EffectComposer` chain — `RenderPass` → flare generation → `UnrealBloomPass` → flare composite → text overlay → colour grade (`ShaderPass`). The composer is bypassed entirely while every effect sits at its default, so the untouched app renders straight to the canvas at no cost; the bloom pass is `enabled = false` until Glow leaves zero. **Chain order is the mechanism behind FR-7k**: the overlay is composited after the bloom (so Glow cannot reach the caption) and before the grade (so Color still does).
...
- Lens flare: one `Pass` subclass owning two quarter-resolution half-float buffers, plus a full-resolution composite. It sits **before** the bloom so it reads the raw scene, writes only its own buffers (`needsSwap = false`) and leaves the chain untouched; the composite is a separate pass **after** the bloom so the flare lands under the text overlay. Four steps: threshold (per tap, at full resolution, then combined with `max` — averaging first throws away thin lines, which is most of what these presets draw, and the per-tap clamp to 1 is what makes `max` safe and keeps presets several stops apart comparable), features (ghosts + anamorphic streak, chromatically split, no halo ring — see FR-7f), separable blur, then an additive composite through a `f/(1+0.7f)` roll-off so a hot preset cannot blow the frame out. Its thresholds are calibrated from measured scene luminance (the set peaks around 0.5–0.9 linear), not from a nominal 0–1 range. The composite's flare uniform is bound **after** `ShaderPass` construction: `cloneUniforms()` substitutes null for a render target's texture rather than cloning it.
- Text overlay: a screen-space scene of its own — two `PlaneGeometry` quads (halo, then glyphs) under an orthographic camera spanning the viewport **in CSS pixels**, so the size control is simply the quad's height and nothing has to be re-derived from the active preset's fov or zoom. It is composited by a small custom `Pass` (`needsSwap = false`, `autoClear` off) that draws straight into the composer's read buffer at the slot described above; with the post chain bypassed, the same scene is drawn over the finished frame directly instead.
- Text glow: painted, not post-processed. The caption is drawn again into a companion canvas through **four canvas-2D shadows at shrinking radii** (1.0 / 0.46 / 0.19 / 0.07 of the slider radius, rising alpha), accumulated with `globalCompositeOperation = 'lighter'` and laid under the glyphs on an additive quad — the same layered-additive trick the Waveform preset uses on its trace. One blur, or the same blur stacked, gives an evenly lit collar that reads as a sticker; light falling off at several rates at once is what makes it read as a lit object. Only the shadows are wanted, so the glyphs are drawn well off the left edge and the shadow offset back on — painting them into the halo as well would stack four opaque copies under the crisp layer and burn the core out to white. Canvas padding grows with the widest radius (~1.3x) or the halo clips at the edge; keeping the halo on its own quad lets its intensity pulse per frame with no redraw. Both canvases share one size, so the two quads stay registered at any scale.
- Text canvases: redrawn on every keystroke, and both `CanvasTexture`s are **disposed whenever the canvas changes size** — GPU storage for a canvas texture is immutable and sized by its first upload, so a growing caption otherwise uploads into the old allocation and renders as garbage. Glyphs are drawn at 2x supersample and tagged `SRGBColorSpace`. Redraws are coalesced to one per frame: a slider drag fires several input events between frames and the glow is the expensive part of a redraw.
