...
**Presets:** Spectrum Tunnel · Radial Spectrum Burst · Frequency Terrain · Waveform Ribbon · Starfield Warp
...
- **Controls:** a collapsible right-hand panel for preset selection, zoom (30%–600%), mic on/off, and live signal meters. Keyboard: `H` panel, `F` fullscreen, `1`–`5` presets, arrows next/prev, `+` / `-` / `0` zoom.
...
Desktop browsers only (WebGL2, Web Audio, `getUserMedia`). Open [index.html](index.html) — it is a single self-contained page that loads Three.js from a CDN, so first load needs network access.
