...
- **Effects:** four global controls that layer over any preset — **Glow** (bloom, 0 = off, and it reaches the preset only, never the caption), **Flare** (lens flare, 0 = off), **Move** (extra random drift on top of the preset's own motion, 0 = off) and **Color** (saturation + brightness together, 0–200%, 100% = the preset as authored).
- **Lens flare:** whatever a preset draws brightly throws the artifact a real lens would — ghosts mirrored through the centre of frame and an anamorphic streak, chromatically split and swelling on onsets. It is derived from the frame itself, so it needs nothing authored per preset and a preset with nothing hot in it produces nothing. Built from the scene *before* the bloom, so Glow and Flare stay independent rather than one feeding the other.
